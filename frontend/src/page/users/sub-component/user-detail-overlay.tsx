      ...
      <div className='fixed right-0 top-0 z-50 h-full w-full max-w-[50%] animate-in slide-in-from-right bg-white dark:bg-surface-dark shadow-2xl duration-300'>
        <div className='flex h-full flex-col'>
          <div className='flex items-center justify-between border-b border-slate-100 dark:border-white/20 p-[24px]'>
            <h3 className='text-[20px] font-bold text-theme-secondary dark:text-white'>User Profile</h3>
            <button 
              onClick={() => dispatch({ _tag: 'SelectUser', user: O.none })}
          ...
          <div className='flex-grow overflow-y-auto p-[32px]'>
            <div className='mb-[32px] flex items-center gap-[24px]'>
              ...
              />
              <div>
                <div className='text-[24px] font-bold text-theme-secondary dark:text-white'>{user.username}</div>
                <div className='text-slate-500 dark:text-slate-200'>{user.email}</div>
              </div>
            </div>
            ...
            <div className='mt-[16px]'>
              <div className='mb-[8px] text-[12px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-200'>Biography</div>
              <div className='rounded-[12px] border border-slate-100 dark:border-white/20 bg-slate-50 dark:bg-black/20 p-[20px] text-[15px] leading-relaxed text-slate-600 dark:text-slate-200 italic'>
                "{user.bio || 'This user has not written a bio yet.'}"
              </div>
            </div>

            <div className='mt-[32px] rounded-[16px] bg-theme-primary/10 p-[24px]'>
              <div className='font-bold text-theme-primary'>Account Meta</div>
              <div className='mt-[12px] flex flex-col gap-[8px] text-[14px] text-theme-primary/80'>
